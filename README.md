...
This module contains some common approaches for building Immunx Agents. You will also find some tools for writing tests for these agents. These approaches can be composed for creating more complex agents or used only for checking without returning their findings.

## Installation

- Using npm: `npm i immunx-agent-tools`
...
Handlers are approaches for dealing with block and transaction events. They can either be integrated into a sensor's logic
to make it easier to get specific data based on transactions or blocks or used like Immunx sensor generators through.
...
this case, it can both return findings through `handler.handle(event)` and also generate the Immunx sensor handlers through
...
- `getHandleBlock(onFinding?)`: This method returns a `immunx-agent` `HandleBlock` handle callback. `onFinding` will override `options.onFinding` if both were specified.
- `getHandleTransaction(onFinding?)`: This method returns a `immunx-agent` `HandleTransaction` handle callback. `onFinding` will override `options.onFinding` if both were specified.
...
```ts
import { Finding, FindingSeverity, FindingType, TransactionEvent } from "immunx-agent";
import { handlers, createAddress } from "immunx-agent-tools";
...
- `sensor`: It is a JS object with two properties, `handleTransaction` and `handleBlock`.
- `block`: It is the `BlockEvent` that the sensor will handle.
- `tx#`: These are the `TransactionEvent` objects asociated with the `BlockEvent` that the sensor will handle.
